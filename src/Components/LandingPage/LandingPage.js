...
const LandingPage = () => {
  ...
  const getNextDayWeather = (weatherData) => {
    ...
    const nextFourDaysWeather = weatherData.list.filter((data) => {
      if ((new Date(data.dt * 1000) - currentDate) / (1000 * 60 * 60) === 24) {
        return { ...data, id: Math.random() };
      }
      if ((new Date(data.dt * 1000) - currentDate) / (1000 * 60 * 60) === 48) {
        return { ...data, id: Math.random() };
      }
      if ((new Date(data.dt * 1000) - currentDate) / (1000 * 60 * 60) === 72) {
        return { ...data, id: Math.random() };
      }
      if ((new Date(data.dt * 1000) - currentDate) / (1000 * 60 * 60) === 96) {
        return { ...data, id: Math.random() };
      }
    });
  ...
  return (
    <header className={classes.header}>
      <div
        ...
        style={{ justifyContent: !isSearched ? "center" : "space-around" }}
      >
